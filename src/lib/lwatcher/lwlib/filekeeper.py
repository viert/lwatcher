...
class FileKeeper(object):
  ...
  def read(self, filename):
    ...
    if filename in self.filestore.keys():
      logging.debug('[FileKeeper] file "%s" metadata found in filestore' % filename)
      ...
      try:
        ...
        logging.debug('[FileKeeper] got new stat: %s' % str(new_stat))
      except OSError, e:
        logging.debug('[FileKeeper] got OSError while getting new_stat for %s: %s' % (filename, str(e)))
      ...
      if new_stat.st_ino != stat.st_ino or new_stat.st_size < f.tell():
        ...
        logging.debug('[FileKeeper] new_stat inode != old_stat inode')

        try:
          logging.debug('[FileKeeper] opening new stated file')
          f = open(filename)
        except OSError, e:
          logging.debug('[FileKeeper] got OSError while opening new file %s: %s' % (filename, str(e)))
        ...
        logging.debug('[FileKeeper] new_stat and file descriptor saved')
      ...
      logging.debug('[FileKeeper] seeking to the end of %s' % filename)
      f.seek(stat.st_size)
      self.filestore[filename] = (stat, f)
      logging.debug('[FileKeeper] stat and file descriptor saved for "%s"' % filename)
      return ''